      ...
      <form class="form" method="POST" action="{{ route('login') }}">
        ...
        <div class="card card-login card-hidden mb-3">
          ...
          <div class="card-body">
            ...
            <div class="form-check mr-auto ml-3 mt-3">
              <label class="form-check-label">
                <input class="form-check-input" type="checkbox" name="remember" {{ old('remember') ? 'checked' : '' }}> {{ __('Recordarme Contraseña') }}
          ...
          <div class="card-footer justify-content-center">
            <button type="submit" class="btn btn-primary btn-link btn-lg">{{ __('Siguiente') }}</button>
          </div>
        </div>
      </form>
      <div class="row">
        <div class="col-6">
            @if (Route::has('password.request'))
                <a href="{{ route('password.request') }}" class="text-light">
                    <small>{{ __('Olvido Contraseña') }}</small>
                </a>
            @endif
        </div>
