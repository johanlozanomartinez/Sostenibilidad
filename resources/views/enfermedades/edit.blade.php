                      ...
                      <ul class="nav">
                        ...
                        <li class="nav-item  ">
                          <a class="nav-link" href="{{ route('cobertura.index') }}">
                            ...
                            <span class="sidebar-normal"> {{ __('Cobertura Vegetal ') }} </span>
                          </a>
                        </li>
                        ...
                        <li class="nav-item   ">
                          <a class="nav-link" href="{{ route('enfermedades.index') }}">
                            <i><img style="width:25px" src="{{ asset('material') }}/img/regist_finc.png"></i>
                            <span class="sidebar-normal"> {{ __('Plagas y Enfermedades') }} </span>
                          </a>
                        </li>
                        ...
                        <li class="nav-item  active ">
                          <a class="nav-link" href="{{ route('agua.index') }}">
                            <i><img style="width:25px" src="{{ asset('material') }}/img/regist_finc.png"></i>
                            <span class="sidebar-normal"> {{ __('Agua por Superficie') }} </span>
                          </a>
                        </li>
                        ...
                        <li class="nav-item   ">
                          <a class="nav-link" href="{{ route('cobertura.index') }}">
                            <i><img style="width:25px" src="{{ asset('material') }}/img/regist_finc.png"></i>
                            <span class="sidebar-normal"> {{ __('Calidad del Suelo ') }} </span>
                          </a>
                        </li>
                    ...
                    <div class="collapse show" id="laravelExample">
                      <ul class="nav">
                        ...
                        <li class="nav-item  ">
                          <a class="nav-link" href="{{ route('agua.index') }}">
                            <i><img style="width:25px" src="{{ asset('material') }}/img/regist_finc.png"></i>
                            <span class="sidebar-normal"> {{ __('Riqueza y Diversidad ') }} </span>
                          </a>
                        </li>
                        ...
                        <li class="nav-item  ">
                          <a class="nav-link" href="{{ route('semillas.index') }}">
                            <i><img style="width:25px" src="{{ asset('material') }}/img/regist_finc.png"></i>
                            <span class="sidebar-normal"> {{ __(' Uso de Semillas Locales') }} </span>
                          </a>
                        </li>
  ...
  <div class="container-fluid">
    <div class="row">
      <div class="col-md-12">
          <div class="card">
            <div class="card-header card-header-primary">
              <h4 class="card-title ">4) PRESIÓN DE PLAGAS Y ENFERMEDADES</h4>

            </div>
            <div class="card-body">
            
          
              <form action="{{route('enfermedades.update',$Enfermedades->id)}}"  method="POST">
                @method('PATCH')
                @csrf
      ...
      <div class="form-group">
        ...
        <div class="form-group">
          ...
          <select name="id_unidad" class="form-control">
            <option disabled>Seleccione la unidad Productiva</option>
            @foreach($ListaUnidad as $und)
            <option value="{{$und->id_unidad}}"> {{$und->nombre_finca}} - Nombre Representante : {{$und->nombre_representante}} </option>
            @endforeach
          </select>
        </div>
      ...
      <div class="form-group">
        ...
        <input type="text" step="any" class="form-control"  name="career_id"   value="{{$Enfermedades-> career_id}}">
      </div> 

      
        <div class="form-group">
          <label>Valor Referencia Umbral de Daño</label>
          <input type="number" step="any" class="form-control"  name="valor_referencia" min="0" max="10000" required value="{{$Enfermedades-> valor_referencia}}">
        </div>   
    
        <div class="form-group">
          <label>Promedio (Captura/Infestación)</label>
          <input type="number" step="any" class="form-control"  name="promedio" min="0" max="10000" required value="{{$Enfermedades-> promedio}}">
        </div>  
      </div>
        ...
        <script>
          $(document).ready(function(){
              ...
              var faculty_id =  $('#faculty_id').val();
                  ...
                  if($.trim(faculty_id) != ''){
                      //Primero(el que recibe el controlador) y segundo (es mediante el id )
                      //Ciudades lo que voy a recibir en el controlador y va ser un array con las ciudades
                      $.get('careers',{faculty_id : faculty_id},function(Creaers){
                          console.log(Creaers);
                          $('#id').empty();//Limpiando el select, para limpiar la data previamente seleccionada
                          $('#id').append("<option value=''>Seleccione enfermedadas</option>");
                      $.each(Creaers,function(create,value){
                          $('#id').append("<option value='" + create + "'"+">" + value +"</option>");
          ...
          CargarCarrera();
        //Se va volver a ejecutar nuevamente la lista desplegable
              $('#faculty_id').on('change',CargarCarrera);
          });
        </script>
